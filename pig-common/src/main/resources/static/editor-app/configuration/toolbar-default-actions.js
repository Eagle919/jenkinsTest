...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        deploy: function (services) {

            var modelMetaData = services.$scope.editor.getModelMetaData();

            var modelUrl = KISBPM.URL.deploy(modelMetaData.modelId)

            var param  = {
                'protocol' : services.$location.protocol(),
                'host' : services.$location.host(),
                'port' : services.$location.port(),
                'absStr' : services.$location.absUrl()
            }

            modelUrl = reBuildeUrl(param,modelUrl);
        ...
        /*start: function (services) {
            ...
            services.$http({    method: 'PUT',
                ...
                url: KISBPM.URL.start(modelMetaData.modelId)
                })
                .success(function (data) {
                    console.log( JSON.stringify(data));
                    window.location.href = "./";
        ...
        closeEditor: function(services) {
            ...
            var modelUrl = KISBPM.URL.delModel(modelMetaData.modelId);
...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    if( modelMetaData.key != null &&  modelMetaData.key != 'undefined'){
    ...
    $scope.save = function (successCallback) {
        ...
        var modelUrl = KISBPM.URL.putModel(modelMetaData.modelId);
        ...
        $http({    method: 'put',
            ...
            //不知道什么原因过了网关后SVG数据传递不过 只能暂时这样写
            var modelResourceUrl = KISBPM.URL.saveModelResouce(modelMetaData.modelId);
            modelResourceUrl = reBuildeUrl(param,modelResourceUrl);

            $http.get(modelResourceUrl,{
                params: {
                    svgData:svgDOM
                }
                ...
                .error(function (data, status, headers, config) {
                    $scope.error = {};
                    console.log('Something went wrong when updating the process modelResource:' + JSON.stringify(data));
                    $scope.status.loading = false;
                });
