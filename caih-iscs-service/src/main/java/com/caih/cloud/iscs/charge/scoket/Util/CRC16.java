...
public class CRC16 {
	 ...
	 * @param data
	 *            需要计算的数组
	 * @return CRC16校验值 0A 03 00 00 00 02 C5 70
	 */
	public static String calcCrc16(String data) {
		String[] arr = data.split(" ");
		byte[] arr2 = new byte[arr.length];
		for(int i = 0;i<arr.length;i++){
			arr2[i] = (byte) (0x00 + Byte.valueOf(arr[i]));
		}
		int crc = calcCrc16(arr2, 0, arr2.length);
		String hex = String.format("%04x", crc);
		return hex.substring(2, 4).toUpperCase() + hex.substring(0, 2).toUpperCase();
	}
